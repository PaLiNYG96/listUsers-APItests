...
let total;
let defaultPerPage;
    it('GET /users', async () => {  // Get actual data to test against
    ...
    it(`GET /users?page=num`, async () => { // GET users by page number
    ...
    it(`GET /users?page=0`, async () => { // Test page=0
        const res = await request
        .get('users?page=0')
    ...
    it('GET /users?page=pageCount + 1', async () => { // Check other pages are empty and dont return errors
        const res = await request
    ...
    it('GET /users?per_page=total', async () => { // All rows on one page
        const res = await request
        .get(`users?per_page=${total}`)
    ...
    it('GET /users?per_page=total/2', async () => { // make page show half the total, should yeild 2 pages 
        const res = await request
        .get(`users?per_page=${total/2}`)
    ...
    it('GET /users?page=3&per_page=total/3', async () => {// make page show one third the total, should yeild 3 pages 
        const res = await request
        .get(`users?page=3&per_page=${total/3}`)
    ...
    it('GET /users/:id', async () => { // Get user by ID
        for(let i=1;i<total+1;i++){ // Loops for all available rows
        const res = await request
        .get(`users/${i}`)
...
describe.only('listUsers - negative testing', async () => {
    ...
    let total;
    let defaultPerPage;
        it('GET /users', async () => { // Get available data to test against
            const res = await request
            .get('users')
        ...
        it('GET /users/:id = invalid', async () => { // List single user (invalid id)
            const res = await request
            .get(`users/${total+1}`)
